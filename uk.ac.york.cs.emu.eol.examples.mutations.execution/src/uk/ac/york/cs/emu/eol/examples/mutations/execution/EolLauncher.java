...
import uk.ac.york.cs.emu.eol.examples.mutations.execution.configurations.Configuration;
...
public class EolLauncher implements Runnable {
	...
	public EolLauncher(Map<String, Object> config) {
		...
		if (config.get(Configuration.DEPENDENCY) != null)
			dependencies = ((String) config.get(Configuration.DEPENDENCY)).split(",");
				...
				while (true) {
					...
					if (dependencies != null) {
						for (String dep : dependencies) {
							src = new File(dep);
							dest = new File(temp_dir.getPath() + File.separatorChar + dep.substring(dep.lastIndexOf(File.separatorChar), dep.length()));
							Files.copy(src.toPath(), dest.toPath(), StandardCopyOption.REPLACE_EXISTING);
							if (imported_by != null) {
								if (dest.getName().equals((String) imported_by))
									mainModule = dest;
							}
							src = dest = null;
						}
					...
					for (short num : getInputFilesNumbers()) {
						...
						if (type == EOLCandidate.CONSOLE_TYPE) {
							...
							for (EmfMetaModel mm : getOutputMetamodelUris(loaded_input_metamodes.split(","))) {
								model_path = mm.getName() + "_" + n + "_" + num + ".xmi";
								// File new_f = new File(model_path);
								_model = newEmfModel("M_OUT_" + mm.getName(), mm.getName(), model_path, mm.getMetamodelUri(), false, true);
						...
						EolExecutor exe = new EolExecutor(eol, "mutant: " + mutant_model.getPath());
						...
						try {
							...
							if (e instanceof TimeoutException)
								...
								logger.write(String.format("Unable to execute EOL %s\n", exe.getName()));
							future.cancel(true);
						...
						if (!killed) {
							try {
								...
								logger.write("An exception thrown while saving output models");
								// killed
								killed = true;
							}
						}
	...
	private void registerAndLoadMetamodels() throws Exception {
		...
		for (int i = 0; i < mm_paths.length; i++) {
			mm = new File(mm_paths[i]);
			if (!mm.exists())
				throw new Exception("Unable to find metamodel: " + mm.getPath());
			uri = org.eclipse.emf.common.util.URI.createURI(mm.getPath());
